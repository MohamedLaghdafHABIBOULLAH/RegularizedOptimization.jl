...
Another approach is to merge the nonsmooth term and the indicator function of the constraints into a single nonsmooth function and then apply **PANOC**, which is the strategy we use here.  
Unfortunately, the current implementation of **PANOC** does not allow handling nonsmooth terms together with bound constraints, so we do not include **PANOC** in our comparison for this example.
Moreover, there is no such implementations
