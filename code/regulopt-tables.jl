...
function nb_prox_evals(stats, solver)
  if solver == R2_None || solver == R2DH_Spec_NM || solver == R2DH_Spec ||
     solver == R2DH_DBFGS || solver == R2DH_Andrei || solver == R2DH_PSB
    prox_evals = stats.iter
  elseif solver == LM_R2DH_Spec || solver == LM_R2
    prox_evals = sum(stats.solver_specific[:SubsolverCounter])
  else
    prox_evals = stats.solver_specific[:prox_evals]
  end
  return prox_evals
end
...
function benchmark_table(
  ...
  for (solver, subsolver) in
    ...
    #  x0 = f.meta.x0
        solver_out, obj_history, time_history = solver(f, h, x0) 
    ...
    push!(time_hist, time_history)
    if solver == LM_R2DH_Spec || solver == LM_R2
      fx = solver_out.solver_specific[:Fhist][end]
      hx = solver_out.solver_specific[:Hhist][end]
    else
      fx = solver_out.solver_specific[:smooth_obj]
      hx = solver_out.solver_specific[:nonsmooth_obj]
    end
    # if obj_min > fx + hx
    #   obj_min = fx + hx
    # end
      ...
      a = @pgf PGFPlotsX.Axis(
        {
          ...
          title = "\\bf $legend_name",
          # legend_pos="south east",
        },
        Tuple(l_plots)...,
        Legend(solver_names),
      )
      pgfsave("Résultats/" * pb_name * "-time.pdf", a)
      pgfsave("Résultats/" * pb_name * "-time.tikz", a)
    end
  ...
  pgfsave("Résultats/" * pb_name * "-iter.pdf", a)
  pgfsave("Résultats/" * pb_name * "-iter.tikz", a)
  ...
  for i = 1:n_solvers
    ...
    if solvers[i] == LM_R2DH_Spec || solvers[i] == LM_R2
      fx = solver_out.solver_specific[:Fhist][end]
      hx = solver_out.solver_specific[:Hhist][end]
    else
      fx = solver_out.solver_specific[:smooth_obj]
