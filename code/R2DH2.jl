...
"""
    R2DH2(f, ∇f!, h, options, x0)

A second calling form for `R2DH2` where the objective and gradient are passed as arguments.
...
function R2DH2(
  ...
  hk = h(xk[selected])
  if hk == Inf
    verbose > 0 && @info "R2DH2: finding initial guess where nonsmooth term is finite"
    prox!(xk, h, x0, one(eltype(x0)))
    hk = h(xk[selected])
    hk < Inf || error("prox computation must be erroneous")
    verbose > 0 && @debug "R2DH2: found point where h has value" hk
  end
  ...
  while !(optimal || tired)
    ...
    mks = mk(s)
    @info "norm of Dkσk ", norm(Dkσk)
    @info "isnan(s)", any(isnan, s)
    @info "s(1) ", s[1]
    @info "norm of s ", norm(s)
    @info "norm of nabla f", norm(∇fk)
    @info "mks: ", mks
    if mks == NaN
        error("mks is NaN")
    end
   ...
   # println("mks: ", mks)
    ...
    if (ξ ≤ 0 || isnan(ξ))
        error("R2DH2: failed to compute a step: ξ = $ξ")
    end
  ...
  if verbose > 0
    ...
    elseif optimal
      ...
      #! format: on
      @info "R2DH2: terminating with √(ξ/ν) = $(sqrt_ξ_νInv))"
    end
  end
